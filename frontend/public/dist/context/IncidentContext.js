...
export class IncidentStore {
    ...
    async submitIntake(payload, isDemo = false) {
        ...
        try {
            ...
            const message = err instanceof ApiError && err.details?.issues
                ? `Validation Error: ${err.details.issues.map((i) => `${i.path}: ${i.message}`).join(', ')}`
    ...
    async loadDemoScenario() {
        ...
        const demoPayload = {
            ...
            description: 'Victim received an urgent SMS claiming power disconnection for unpaid electricity bill. Called number provided, and was tricked into accepting a UPI collect request on PhonePe.',
            victim: {
                name: 'Pooja Varma',
                phone: '9845012345',
